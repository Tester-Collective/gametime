...
class GameServiceTest {
    ...
    void testFindGamesBySeller_Positive() {
        when(gameRepository.findGamesBySellerAndGameDeletedOrderByTitle(user, false)).thenReturn(gameList);
    ...
    void testFindGamesBySeller_Negative() {
        ...
        when(gameRepository.findGamesBySellerAndGameDeletedOrderByTitle(user2, false)).thenReturn(empty);
    ...
    @Test
    void testFindGamesByKeyword() {
        when(gameRepository.findGamesByGameDeletedAndTitleContainingIgnoreCaseAndStockGreaterThanOrderByTitle(false, "CS:GO", 0)).thenReturn(gameList);
        List<Game> result = gameService.findGamesByKeyword("CS:GO");
        assertEquals(gameList, result);
    }

    @Test
    void testLazyDeleteGame() {
        game.setGameDeleted(true);
        when(gameRepository.save(game)).thenReturn(game);
        boolean result = gameService.lazyDeleteGame(game);
        assertTrue(result);
    }
}
